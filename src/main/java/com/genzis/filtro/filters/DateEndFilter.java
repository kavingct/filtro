...
/**
 * Filter to check if given date ends with a specified limit.
 * @author Kavin Subramanian
 *
 */
public class DateEndFilter implements Filter<Date> {
	private Date limit;
	
	/**
	 * Construct a DateStartFilter object with the specified end limit.
	 * @param limit End limit.
	 */
	public DateEndFilter(Date limit)
	{
		this.limit = limit;
	}
	...
	public boolean check(Date date) {
		return date.equals(limit) || date.before(limit);
	}
}